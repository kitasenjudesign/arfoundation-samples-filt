...
{
    /// <summary>
    /// InstagramKit Dependencies for Cross Platform Instagram Kit.
    /// </summary>
    [InitializeOnLoad]
    public class InstagramKitLibraryDependencies
    {
		/// <summary>
		/// The name of your plugin.  This is used to create a settings file
		/// which contains the dependencies specific to your plugin.
		/// </summary>
		static readonly string DependencyName 		    = "InstagramKitDependencies.xml";
		static readonly string Path 				    = Constants.kPluginAssetsPath + "/Editor/" + DependencyName;
        static readonly string SupportLibsVersionString = "26.0+";


        /// <summary>
        /// Initializes static members of the <see cref="InstagramKitLibraryDependencies"/> class.
        /// </summary>
        static InstagramKitLibraryDependencies()
		{
			EditorUtils.Invoke(()=>{
				CreateDependencyXML();
			}, 0.1f);
		...
		public static void CreateDependencyXML()
		{
			...
			using (XmlWriter _xmlWriter = XmlWriter.Create(_path, _settings))
			{
				...
				{
                    _xmlWriter.WriteComment("DONT MODIFY HERE. AUTO GENERATED DEPENDENCIES FROM InstagramKitLibraryDependencies.cs.");

					_xmlWriter.WriteStartElement("dependencies");
					{
                        WriteAndroidDependencies(_xmlWriter);
                    }
        ...
        private static void WriteAndroidDependencies(XmlWriter _xmlWriter)
        {
