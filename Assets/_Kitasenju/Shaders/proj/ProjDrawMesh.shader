...
#define _ViewMat_arr Props
			...
			v2f vert (appdata v)
			{
				...
				//v.vertex.xyz = vv.xyz;
				float3 ppos = v.vertex.xyz;
				//ppos.x *= sin(_Time.z)*0.5+0.5;
				//ppos.y *= 0.2;
				o.vertex = UnityObjectToClipPos(ppos);

				float4x4 modelMat = UNITY_ACCESS_INSTANCED_PROP(_ModelMat_arr, _ModelMat);
				float4x4 viewMat =  UNITY_ACCESS_INSTANCED_PROP(_ViewMat_arr, _ViewMat);
				float4x4 projMat =  UNITY_ACCESS_INSTANCED_PROP(_ProjMat_arr, _ProjMat);

				float4 fixV = v.vertex;
				//fixV.x = 0;
				float4 worldPos = mul(modelMat,float4(fixV.xyz, 1.0));
